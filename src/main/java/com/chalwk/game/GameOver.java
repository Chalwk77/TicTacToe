...
public class GameOver {
    ...
    private static void endGame(int state, String[] game, MessageReceivedEvent event) {

        String embedID = game[2];
        event.getChannel().retrieveMessageById(embedID).queue(message -> message.delete().queue());

        EmbedBuilder embed = new EmbedBuilder();
        embed.setTitle("Tic-Tac-Toe - Game Over");
        if (state == 0) {
            embed.setFooter("Draw");
        } else if (state == 1) {
            embed.setFooter("Player 1 wins");
        } else {
            embed.setFooter("Player 2 wins");
        }

        event.getChannel().sendMessageEmbeds(embed.build()).queue();
    }

    public static boolean gameOver(char[][] board, String[] game, MessageReceivedEvent event) {
        int count = 0;
        ...
        if (count == board.length * board.length) {
            endGame(0, game, event);
            return true;
        }
        if (getWinner(board, player1)) {
            endGame(1, game, event);
            return true;
